'use client';

import { Slider as SliderPrimitive } from '@base-ui/react/slider';
import { cn } from '@workspace/shadcn/lib/utils';
import * as React from 'react';

function Slider({
  className,
  defaultValue,
  value,
  min = 0,
  max = 100,
  ...props
}: SliderPrimitive.Root.Props) {
  const _values = React.useMemo(
    () =>
      value === undefined || typeof value === 'number'
        ? defaultValue === undefined || typeof defaultValue === 'number'
          ? [min, max]
          : defaultValue
        : value,
    [value, defaultValue, min, max],
  );

  return (
    <SliderPrimitive.Root
      className={`
        data-horizontal:w-full
        data-vertical:h-full
      `}
      data-slot="slider"
      defaultValue={defaultValue}
      value={value}
      min={min}
      max={max}
      thumbAlignment="edge"
      {...props}
    >
      <SliderPrimitive.Control
        className={cn(
          `
            relative flex w-full touch-none items-center select-none
            data-disabled:opacity-50
            data-vertical:h-full data-vertical:min-h-40 data-vertical:w-auto
            data-vertical:flex-col
          `,
          className,
        )}
      >
        <SliderPrimitive.Track
          data-slot="slider-track"
          className={`
            relative overflow-hidden rounded-full bg-muted select-none
            data-horizontal:h-1.5 data-horizontal:w-full
            data-vertical:h-full data-vertical:w-1.5
          `}
        >
          <SliderPrimitive.Indicator
            data-slot="slider-range"
            className={`
              bg-primary select-none
              data-horizontal:h-full
              data-vertical:w-full
            `}
          />
        </SliderPrimitive.Track>
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb
            data-slot="slider-thumb"
            key={index}
            className={`
              block size-4 shrink-0 rounded-full border border-primary bg-white
              shadow-sm ring-ring/50 transition-[color,box-shadow] select-none
              hover:ring-4
              focus-visible:ring-4 focus-visible:outline-hidden
              disabled:pointer-events-none disabled:opacity-50
            `}
          />
        ))}
      </SliderPrimitive.Control>
    </SliderPrimitive.Root>
  );
}

export { Slider };
